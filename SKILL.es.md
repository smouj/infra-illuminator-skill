...
description: Descubre dependencias ocultas y relaciones en infraestructura distribuida
version: "1.0.0"
...
Descubre dependencias ocultas y relaciones en sistemas distribuidos, generando mapas de dependencias visuales e identificando puntos únicos de falla.

## Purpose

**Primary Use Cases:**

- **Service Dependency Mapping**: Descubre acoplamientos implícitos entre microservicios analizando logs de tráfico, archivos de configuración y políticas de red. Ejemplo: Identifica que `payment-service` depende indirectamente de `legacy-auth` a través de `notification-worker` tras rastrear 3 saltos de conexiones de cola de mensajes.

- **Cloud Resource Inventory**: Analiza archivos de estado de Terraform y AWS Config para mapear relaciones entre VPCs, subnets, security groups, roles IAM y almacenes de datos. Ejemplo: Encuentra que 12 funciones Lambda comparten un único rol de ejecución IAM con acceso S3 excesivamente permisivo.

- **Configuration Drift Detection**: Compara infraestructura en vivo contra definiciones de IaC para identificar recursos o relaciones no gestionados. Ejemplo: Descubre 3 instancias EC2 con reglas de security group no definidas en Terraform.

- **Single Point of Failure Analysis**: Calcula scores de criticidad para cada nodo en el grafo de dependencias basándose en el impacto descendente. Ejemplo: Marca que `shared-redis-cluster` sirve a 8 servicios críticos sin redundancia.

- **Impact Analysis**: Antes de despliegues, traza todos los sistemas descendentes que podrían verse afectados por un cambio. Ejemplo: Determina que actualizar `user-api` impactará 5 servicios frontend, 2 apps móviles y 15 workers de fondo.
